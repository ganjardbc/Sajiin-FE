        ...
        <div class="left">
            <div class="bg-white">
                <div class="display-flex row space-between border-bottom padding padding-10-px" style="height: 40px;">
                    <div>
                        <h1 class="fonts small black">USERS</h1>
                    ...
                    <div class="display-flex">
                        <AppButtonMenu 
                            ...
                            :data="[{label: 'By ID'}, {label: 'By Name'}, {label: 'By Username'}, {label: 'By Status'}]" />
                        <button class="btn btn-white btn-icon btn-radius" @click="onShow('CREATE')">
                            <i class="fa fa-lw fa-plus" />
                        </button>
                        <SearchField :placeholder="'Search users ..'" :enableResponsive="true" style="margin-left: 5px;" />
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async saveData () {
            ...
            const token = 'Bearer '.concat(this.$cookies.get('token'))
            const payload = this.selectedData
            const url = this.formTitle === 'CREATE' ? '/api/user/post' : '/api/user/update' 
        ...
        async getData () {
            ...
            const rest = await axios.post('/api/user/getAll', payload, { headers: { Authorization: token } })
