        ...
        <div class="left">
            <div class="bg-white box-shadow">
                <div class="display-flex row space-between border-bottom padding padding-10-px" style="height: 40px;">
                    <div>
                        <h1 class="fonts small black">VISITORS</h1>
                    ...
                    <div class="display-flex">
                        ...
                        <SearchField :placeholder="'Search visitors ..'" :enableResponsive="true" style="margin-left: 5px;" />
                    </div>
                </div>
                ...
                <div class="table-container">
                    <v-table 
                        ...
                        <thead slot="head">
                            ...
                            <v-th sortKey="visitor.start_date">Start Date</v-th>
                            <v-th sortKey="visitor.end_date">End Date</v-th>
                            <v-th sortKey="customer.name">Customer</v-th>
                            <v-th sortKey="user.name">Table</v-th>
                        ...
                        <tbody slot="body" slot-scope="{displayData}">
                            ...
                            <tr v-for="(row, index) in displayData" :key="index">
                                ...
                                <td>{{ row.visitor.start_date }}</td>
                                <td>{{ row.visitor.end_date }}</td>
                                <td>{{ row.customer.name }}</td>
                                <td>{{ row.table.name }}</td>
                                <td class="normal-col">
                                    <div 
                                        :class="'card-capsule ' + (row.visitor.status === 'active' ? 'active' : '')" 
                                ...
                                <td class="medium-col">
                                    <div class="display-flex justify-content">
                                        <button class="btn btn-transparent btn-small-icon btn-radius" @click="onShow('EDIT', row.visitor.id)">
                                            <i class="fa fa-lw fa-pencil-alt" />
                                        </button>
                                        <button class="btn btn-transparent btn-small-icon btn-radius" @click="onShowHideDelete(row.visitor.id)">
                                            <i class="fa fa-lw fa-trash-alt" />
                                        </button>
                                        <button class="btn btn-transparent btn-small-icon btn-radius" @click="onShow('VIEW', row.visitor.id)">
...
export default {
    ...
    methods: {
        ...
        async removeData () {
            ...
            const id = this.onSearchData(this.selectedIndex).visitor.id
            const payload = {
                id: id
            }

            const rest = await axios.post('/api/visitor/delete', payload, { headers: { Authorization: token } })
        ...
        async saveData () {
            ...
            const url = this.formTitle === 'CREATE' ? '/api/visitor/post' : '/api/visitor/update' 
