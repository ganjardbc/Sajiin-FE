        ...
        <div class="left">
            <div class="bg-white box-shadow">
                <div class="display-flex row justify-content border-bottom padding padding-10-px" style="height: 40px;">
                    <div style="width: 100%;">
                        <h1 class="fonts small black">UNCONFIRMED</h1>
                ...
                <div class="table-container">
                    <v-table 
                        ...
                        <thead slot="head">
                            ...
                            <v-th sortKey="total_price">Total Price</v-th>
                            <!-- <v-th sortKey="total_item">Total Item</v-th>
                            <v-th sortKey="delivery_fee">Delivery Fee</v-th> -->
                            <v-th sortKey="payment_status">Payment Status</v-th>
                            <v-th sortKey="status" class="normal-col">Order Status</v-th>
                        ...
                        <tbody slot="body" slot-scope="{displayData}">
                            ...
                            <tr v-for="(row, index) in displayData" :key="row.order.id">
                                <td class="small-col">{{ row.order.id }}</td>
                                <td>{{ row.order.order_id }}</td>
                                <!-- <td>{{ row.order.total_price }}</td>
                                <td>{{ row.order.total_item }}</td> -->
                                <td>{{ row.order.delivery_fee }}</td>
                                <td>{{ row.order.payment_status ? 'Paid' : 'Unpaid' }}</td>
                                <td class="normal-col">{{ row.order.status }}</td>
                                <td class="medium-col">
...
export default {
    ...
    methods: {
        ...
        async removeData () {
            ...
            const id = this.datas[this.selectedIndex].order.order_id
            const payload = {
                order_id: id
            }
